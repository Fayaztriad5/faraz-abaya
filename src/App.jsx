...
function Home({ products, onView, cat, setCat, search }) {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 20px" }}>
      {/* Hero */}
      <div style={{ borderRadius: 28, overflow: "hidden", background: "linear-gradient(135deg,#1C1C1E 55%,#2C1F0E 100%)", position: "relative", marginBottom: 40, minHeight: 240 }}>
        ...
        <div style={{ position: "relative", zIndex: 1, padding: "48px 40px", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 24 }}>
          <div>
            <div style={{ fontSize: 11, color: "var(--gold)", letterSpacing: "0.25em", textTransform: "uppercase", fontFamily: "'Jost',sans-serif", fontWeight: 700 }}>New Eid Collection 2025</div>
            <div className="font-display" style={{ fontSize: 52, fontWeight: 600, color: "#FAF8F5", lineHeight: 1.1, margin: "10px 0 12px" }}>
              Modesty,<br /><span style={{ color: "var(--gold-light, #E8C96A)" }}>Redefined.</span>
            </div>
            <p style={{ color: "rgba(250,248,245,.6)", fontFamily: "'Jost',sans-serif", fontSize: 14, maxWidth: 340, lineHeight: 1.75 }}>
      ...
      {search && <div style={{ fontFamily: "'Jost',sans-serif", fontSize: 13, color: "var(--text-muted)", marginBottom: 18 }}>Showing {filtered.length} result{filtered.length !== 1 ? "s" : ""} for "<strong style={{ color: "var(--charcoal)" }}>{search}</strong>"</div>}

      {/* Grid */}
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "80px 0" }}>
          <div style={{ fontSize: 48 }}>🕌</div>
          <div className="font-display" style={{ fontSize: 24, marginTop: 12, color: "var(--charcoal)" }}>No products found</div>
      ...
      {/* Features */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))", gap: 14, marginTop: 48 }}>
        {[["🪡", "Premium Fabrics", "Nida, Crepe, Chiffon"], ["📏", "Sizes 52–58", "Perfect fit guarantee"], ["🚚", "Fast Delivery", "Pan-India 3–5 days"], ["💬", "WhatsApp Order", "No app needed"]].map(([icon, t, s]) => (
          <div key={t} style={{ background: "#fff", borderRadius: 18, padding: 20, textAlign: "center", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
            <div style={{ fontSize: 28 }}>{icon}</div>
            <div className="font-display" style={{ fontSize: 16, fontWeight: 600, color: "var(--charcoal)", margin: "6px 0 2px" }}>{t}</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", fontFamily: "'Jost',sans-serif" }}>{s}</div>
          </div>
        ))}
      </div>
    </div>
...
function Footer({ onNav }) {
  return (
    <footer style={{ background: "var(--charcoal)", color: "rgba(250,248,245,.65)", marginTop: 60 }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "48px 20px 24px" }}>
        ...
        <div style={{ height: 1, background: "linear-gradient(90deg,transparent,var(--gold),transparent)", margin: "0 0 20px" }} />
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--pearl)" }}>
      ...
      {page !== "admin" && (
        <button className="float-wa" onClick={() => window.open(`https://wa.me/${WHATSAPP}`, "_blank")}>
          <WaIcon size={26} />
        </button>
      )}
    </div>
