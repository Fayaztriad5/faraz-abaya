...
export default function ProductDetail({ p, onBack }) {
  ...
  return (
    <div className="fade-up" style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 20px" }}>
      <button onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", cursor: "pointer", fontFamily: "'Jost',sans-serif", fontSize: 14, color: "var(--text-muted)", marginBottom: 28 }}>
        ← Back to Collections
      </button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 48 }}>
        {/* Gallery */}
        <div>
          <div style={{ borderRadius: 20, overflow: "hidden", aspectRatio: "3/4", marginBottom: 12 }}>
            <img src={p.imgs[img]} alt={p.name} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "all .5s" }} />
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            {p.imgs.map((src, i) => (
              <div key={i} className={`thumb ${img === i ? "active" : ""}`} onClick={() => setImg(i)} style={{ flex: 1, height: 80 }}>
        ...
        <div>
          ...
          <div className="font-display" style={{ fontSize: 40, fontWeight: 600, color: "var(--charcoal)", lineHeight: 1.15, margin: "6px 0 10px" }}>{p.name}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <Stars r={p.rating} />
            <span style={{ fontSize: 13, color: "var(--text-muted)", fontFamily: "'Jost',sans-serif" }}>{p.rating} · {p.reviews} reviews</span>
          </div>
          <div className="divider" />
          <div className="font-display" style={{ fontSize: 38, fontWeight: 600, color: "var(--charcoal)", marginBottom: 4 }}>₹{p.price.toLocaleString()}</div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              {["52", "54", "56", "58"].map(s => (
                <button key={s} className={`size-btn ${size === s ? "active" : ""}`} onClick={() => setSize(s)} style={{ width: 56, height: 56, borderRadius: 14 }}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
            <button className="btn-wa" onClick={order} style={{ width: "100%", padding: 16, borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontSize: 15, fontFamily: "'Jost',sans-serif", fontWeight: 600 }}>
